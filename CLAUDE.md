...
- **Chrome:** `components/admin/admin-nav.tsx` is a client component (`usePathname` for active-link highlight) listing 8 planned sections (`/admin`, `/admin/moderation`, `/admin/feedback`, `/admin/metrics`, `/admin/users`, `/admin/content`, `/admin/ops`, `/admin/push`) — only `/admin` (Genel Bakış placeholder) exists so far; the others 404 until their own P1+ tasks land. Don't treat a 404 under `/admin/*` as a bug without checking whether that section has shipped yet.
...
- **`.rpc(name).returns<T[]>()` can fail `next build` with a fake "Type mismatch: Cannot cast single object to array type" error** on some RPC shapes (hit on `admin_report_queue`/`admin_feedback_list` in `lib/admin/{moderation,feedback}.ts`), even though the RPC genuinely returns an array. Not universal — `browse_adoptions` etc. use `.returns<T[]>()` fine. If you hit this, drop `.returns<T[]>()` and cast `data as RowType[]` after the error/null check instead.
