...
import { EmergencyFilterBar } from '@/components/emergency/emergency-filter-bar';
import { EmergencyCard } from '@/components/emergency/emergency-card';
import { Button } from '@/components/ui/button';
import { loadBrowseEmergencyAction, loadNearbyEmergencyAction } from '@/lib/emergency/actions';
import {
  hasActiveFilters,
  loadFilterSnapshot,
  saveFilterSnapshot,
  withRadius,
} from '@/lib/emergency/filters';
import { EMPTY_EMERGENCY_FILTERS, PER_PAGE } from '@/lib/emergency/types';
import type { EmergencyFilters, EmergencyListing } from '@/lib/emergency/types';
...
// mount (Task 5 brief, mirrors Adoptions' browse-list).
export function EmergencyBrowseList({
  initial,
  ownerId,
}: {
  initial: EmergencyListing[];
  ownerId: string | null;
}) {
  const [filters, setFilters] = useState<EmergencyFilters>(EMPTY_EMERGENCY_FILTERS);
  ...
  async function fetchFirstPage(next: EmergencyFilters) {
    setGeoNotice(null);

    if (next.radiusKm == null) {
      setGeoCoords(null);
      const result = await loadBrowseEmergencyAction(next, 0);
      setItems(result);
      setPage(1);
    ...
    try {
      ...
      setGeoCoords(coords);
      const result = await loadNearbyEmergencyAction(coords.lat, coords.long, next, 0);
      setItems(result);
      setPage(1);
      setHasMore(result.length >= PER_PAGE);
    } catch {
      ...
      const result = await loadBrowseEmergencyAction(fallback, 0);
      setItems(result);
      setPage(1);
      setHasMore(result.length >= PER_PAGE);
    }
    ...
    startTransition(async () => {
      const next =
        filters.radiusKm != null && geoCoords
          ? await loadNearbyEmergencyAction(geoCoords.lat, geoCoords.long, filters, page)
          : await loadBrowseEmergencyAction(filters, page);
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          {hasActiveFilters(filters) ? (
            <Button variant="outline" onClick={() => handleFiltersChange(EMPTY_EMERGENCY_FILTERS)}>
              Filtreleri temizle
            </Button>
          ) : (
            <Button asChild>
              <Link href="/emergency/create">Vaka bildir</Link>
            </Button>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          {items.map((listing) => (
            <EmergencyCard key={listing.id} item={listing} />
          ))}
        </div>
      )}
